...
export const CATEGORY_FORMAT_MAP: Record<string, Record<string, string[]>> = {
  ...
  video: {
    mp4:  ['avi', 'mkv', 'webm', 'mov', 'mp3', 'gif', 'zip'],
...
export function getAvailableOutputsForCategory(ext: string, category: string): string[] {
  return CATEGORY_FORMAT_MAP[category]?.[ext] ?? [];
}

export function getAvailableOutputsForFile(file: File, category: string): string[] {
...
export const FORMAT_OPTIONS: Record<string, ConversionOption[]> = Object.fromEntries(
